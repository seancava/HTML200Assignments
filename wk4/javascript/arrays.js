...
function onImageClick(evt){
  var onlyPath = evt.target.getAttribute("src");//stores the source attribute of the image that was clicked
  var imageIndex = imageListA.indexOf(onlyPath);//stores the returned index number where the img src == src stored in path variable
  var selectedImage = imageListA.splice(imageIndex, 1);//image is removed from imageListA array and save clicked image path to a variable//
  purchasedImages.push(selectedImage);//push adds the element to the array
  update_shopping_cart();
  buildThumbnails();//reloads the function which reloads the images stored in imageListA array which will not include the clicked image because is was spliced out of the array  in the selectedImage variable on click
};
